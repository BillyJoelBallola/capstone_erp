...
const SupplyChain = () => {
    ...
    useEffect(() => {
        axios.get("/erp/purchases").then(({ data }) => {
            const pending = data.filter(item => item.state === 1);
            const inProgress = data.filter(item => item.state === 2);
            const waiting = data.filter(item => item.state === 5);
            setPurchase({
                pending: pending.length,
                inProgress: inProgress.length,
                ...
                <div className='grid grid-cols-2 gap-5'>
                    ...
                    <div className='flex flex-col gap-6 bg-white drop-shadow-md p-4 rounded-md'>
                        ...
                        <div className="grid gap-2 font-semibold">
                            ...
                            <div className='bg-cyan-100 items-center py-1 px-3 rounded-lg flex justify-between'>
                                <span className='text-cyan-700'>Waiting Bills</span>
                                <span className='text-cyan-700 text-2xl'>{purchase.waiting}</span>
                            </div>
                        </div>
                    </div>
                </div>
